...
class Nutrition extends CI_Controller
{
    public function getGridRows()
    {
        ...
        $this->grid_model->initiateNutritionData($request);
        $data['status'] = true;
    ...
    {
        verifAccess();
        $this->load->model('web/nutrition_model');
        $result = $this->nutrition_model->getById($whereId);
    ...
    public function createData()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $statusProcess = $this->nutrition_model->insert($dataProcess);
            if ($statusProcess) {
                $response['status'] = true;
    ...
    public function editData()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $statusProcess = $this->nutrition_model->update($whereId, $dataProcess);
            if ($statusProcess) {
                $response['status'] = true;
    ...
    public function getListData()
    {
        ...
        $result = $this->nutrition_model->getListData($rowPerPage, $offset, $search);
        $total = $this->nutrition_model->getListDataTotal($search);
    ...
    public function getMemberByParentId($whereParentId){
        $this->load->model('web/nutrition_model');
        $result = empty($whereParentId)?[]:$this->nutrition_model->getNutritionFoodCatById($whereParentId);
        $response['status'] = true;
        $response['data'] = $result;
        $this->output->set_content_type('application/json')->set_output(json_encode($response));
    }
    ...
    public function addMember(){
        ...
        if(!empty($memberId) && !empty($parentId)){
            if($this->nutrition_model->isFoodCatExistInNutrition($parentId,$memberId)){
                $response['status'] = false;
                $response['error'] = "Data is already exist";
            }else{

                $date = new DateTime('now');
                $dataProcess['nutrition_type_id'] = $parentId;
                $dataProcess['food_cat_id'] = $memberId;
                $dataProcess['iby'] = $this->session->userdata('username');
                $dataProcess['idt'] = $date->format('Y-m-d H:i:s');
                $statusProcess = $this->nutrition_model->insertFoodCat($dataProcess);
                if ($statusProcess) {
                    $resInsert = $this->nutrition_model->getNutritionFoodCatByFoodCatId($parentId,$memberId);
                    $response['status'] = true;
    ...
    public function removeMember(){
        ...
        if(!empty($memberId) && !empty($parentId)){
            $statusProcess = $this->nutrition_model->deleteFoodCat($parentId,$memberId);
            if ($statusProcess) {
                $response['status'] = true;
                $response['data'] = "Data has been deleted";
            } else {
