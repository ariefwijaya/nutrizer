...
// (CASE WHEN b.count_play > 999999 THEN CONCAT(FORMAT(b.count_play/1000000,1),'M') WHEN b.count_play > 999 THEN CONCAT(FORMAT(b.count_play/1000,1),'K') ELSE b.count_play END) countLabel, IF(DATEDIFF(b.idt, CURDATE()) < 14,'NEW','') tagLabel
    public function getGroupTagMemberAlbum($idGroup,$streamUrlImage,$offset=0,$limit=6)
    {
        $sql = "SELECT 'album' type,
            ...
            WHERE a.id_group_tag='".$idGroup."' AND b.isdeleted=0  LIMIT ".$offset.",".$limit;
    ...
    public function getAlbumByArtist($artistId,$streamUrlImage,$offset=0,$limit=6)
    {
        $sql = "SELECT 'album' type,
            a.album_id id,CONCAT('".$streamUrlImage."',b.image) imageUrl,b.album_name title, 'http://musica.tl' linkShare,
            (
            SELECT sub.full_name
            FROM tbl_artist sub
            WHERE sub.id = a.artist_id) subtitle,
            IFNULL((SELECT SUM(sub.count_play) FROM tbl_song sub WHERE sub.album_id=b.id),0) countLabel,
            IF(DATEDIFF(b.idt, CURDATE()) < 14,'NEW','') tagLabel
            FROM tbl_album_artist a
            INNER JOIN tbl_album b ON a.album_id = b.id
            WHERE a.artist_id='".$artistId."' AND b.isdeleted=0  LIMIT ".$offset.",".$limit;
    ...
    function getSongsByQuery($query,$username,$streamUrlImage,$streamUrlImageThumb,$streamUrlAudio,$streamUrlLyric,$offset=0,$limit=6){
        $sql="SELECT
            ...
            CONCAT('".$streamUrlImageThumb."',(SELECT sub.image FROM tbl_album sub WHERE sub.id=a.album_id)) thumbUrl,
    ...
    function getTopSongs($streamUrlImage,$streamUrlImageThumb,$streamUrlAudio,$streamUrlLyric,$offset=0,$limit=6){
        $sql="SELECT
            a.id,
            a.title,
            a.artist_id artistId,
            (         
            SELECT IF(COUNT(sub.id)>1,'Various Artist',sub.full_name)
            FROM tbl_artist sub
            WHERE sub.id = (
            SELECT subx.artist_id
            FROM tbl_album_artist subx
            WHERE subx.album_id=a.album_id
            LIMIT 1)) artistName,
            a.album_id albumId,(SELECT sub.album_name FROM tbl_album sub WHERE sub.id=a.album_id) albumName,
            ROUND(a.duration*1000,0) duration,
             count_like likesCount, 
            ...
            'http://musica.tl' linkShare,
            FALSE isLiked
            FROM tbl_song a WHERE a.isdeleted=0 ORDER BY a.count_play DESC LIMIT ".$offset.",".$limit;

            $query = $this->db->query($sql);
            return $query->result_array();
    }

    function getArtistsByQuery($query,$streamUrlImage,$streamUrlImageThumb,$offset=0,$limit=6){
        $sql="SELECT
            a.id,a.full_name fullName,a.nick_name nickName,
            CONCAT('".$streamUrlImage."',a.image) imageUrl,
            CONCAT('".$streamUrlImageThumb."',a.image) thumbUrl
            FROM tbl_artist a WHERE (a.full_name  LIKE '%".$query."%' OR a.nick_name  LIKE '%".$query."%') and a.isdeleted=0  LIMIT ".$offset.",".$limit;

            $query = $this->db->query($sql);
            return $query->result_array();
    }

    function getAlbumsByQuery($query,$streamUrlImage,$streamUrlImageThumb,$offset=0,$limit=6){
        $sql="SELECT
            a.id,
            (         
            SELECT IF(COUNT(sub.id)>1,'Various Artist',sub.full_name)
            ...
            CONCAT('".$streamUrlImage."',a.image) imageUrl,
            CONCAT('".$streamUrlImageThumb."',a.image) thumbUrl
            FROM tbl_album a WHERE (a.album_name  LIKE '%".$query."%') and a.isdeleted=0  LIMIT ".$offset.",".$limit;
    ...
    public function getGroupTagMemberPlaylist($idGroup,$streamUrlImage,$offset=0,$limit=6)
    {
        ...
        'playlist' type,
                a.id_member id,
                CONCAT('".$streamUrlImage."',b.image) imageUrl,
                b.name title, 
                'http://musica.tl' linkShare,
    ...
    function getAlbumSong($id,$username,$streamUrlImage,$streamUrlImageThumb,$streamUrlAudio,$streamUrlLyric){
        $sql="SELECT
            a.id,a.title,a.artist_id artistId,(SELECT sub.full_name FROM tbl_artist sub WHERE sub.id=a.artist_id) artistName,
            a.album_id albumId,(SELECT sub.album_name FROM tbl_album sub WHERE sub.id=a.album_id) albumName,
            ROUND(a.duration*1000,0) duration,
            ...
            IF((
                    ...
                    FROM tbl_song_fav sub
                    WHERE sub.song_id = a.id AND sub.username='".$username."') >0, TRUE, FALSE) isLiked
            FROM tbl_song a WHERE a.album_id = '".$id."' AND a.isdeleted=0 ";

            $query = $this->db->query($sql);
            return $query->result_array();
    }

     function getPlaylistSong($id,$username,$streamUrlImage,$streamUrlImageThumb,$streamUrlAudio,$streamUrlLyric){
        $sql="SELECT
            a.id,a.title,a.artist_id artistId,(SELECT sub.full_name FROM tbl_artist sub WHERE sub.id=a.artist_id) artistName,
            a.album_id albumId,(SELECT sub.album_name FROM tbl_album sub WHERE sub.id=a.album_id) albumName,
            ROUND(a.duration*1000,0) duration,
    ...
    public function getPlanPacketList()
    {
        ...
        $this->db->order_by("a.validity_seconds");
        $query = $this->db->get();
        return $query->result_array();
    }
