            ...
            <form class="" id="formData" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input name="name" type="text" class="form-control" placeholder="Eg: Sayuran" required>
...
<div class="modal fade" id="formManageMemberModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="formManageMemberModalTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 id="formManageMemberModalTitle" class="modal-title">Food in Food Category</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    function actionCellRenderer(params) {
        if (params.data != undefined && params.data.id != undefined) {
            ...
            return '<div class="buttons">' +
                ...
                '<button onclick="manageMemberModal(this)" data-id="' + idVal + '" data-name="' + nameVal + '" class="btn btn-icon btn-info btn-sm song_modal"  data-toggle="tooltip" title="Add Food In Category"><i class="fas fa-hamburger"></i></button>' +
                '</div>';
        } else {
            return '';
        }
    ...
    function thumbImageRenderer(params) {
        if (params.value != undefined) {
            var valData = encodeURI(params.value);
            return '<a ' + (params.value ? 'href="' + (BASE_URL + 'loader/images/foodCategory/' + valData) + '" target="_blank"' : 'href="javascript:;"') + '> <div class="small-avatar zoomhover" alt="image" style="background-image:url(' + BASE_URL + 'loader/thumb/foodCategory/' + valData + ');" ></div></a>';
        } else {
            return '';
        }
    }
    ...
    var apiGrid = API_URL + "foodCategories";
    var apiAdd = API_URL + "foodCategory/add";
    var apiEdit = API_URL + "foodCategory/edit";
    var apiDelete = API_URL + "foodCategory/delete";
    var apiGet = API_URL + "foodCategory/get/";
    var apiGetMember = API_URL + "foodCategoryFood/";
    var apiAddMemberToParent = API_URL + "foodCategoryFoodAdd";
    var apiDeleteMemberFromParent = API_URL + "foodCategoryFoodDelete";
    var apiMemberSearch = API_URL + 'food/search';
    ...
    $(document).ready(function() {
        ...
        $("#select-member").select2({
            ...
            placeholder: 'Search Food to be added in Food Category',
        ...
        $("button.add_modal").click(function(e) {
            ...
            clearForm();
            formState = "add";
            $("#formModal" + " .modal-title").html('Add New Food Category');
        ...
        $.ajax({
            ...
            success: function(response, status, xhr) {
                if (response.status) {
                    ...
                    listHtml+='</ul>';
                    $("#formManageMemberModal .modal-title").html("Food in Food Category: "+name);
        ...
        $.ajax({
            ...
            success: function(response, status, xhr) {
                if (response.status) {
                    ...
                    for (const key in data) {
                        if (data.hasOwnProperty(key)) {
                            ...
                            if (key == "image" && element != null) {
                                $("#" + key + "_image-preview").attr("style", "background-image: url(" + BASE_URL + "loader/images/foodCategory/" + element + ")");
                    ...
                    $("#formModal" + " .modal-title").html('Edit Food Category');
